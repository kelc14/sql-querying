...
SELECT app_name FROM analytics WHERE last_updated = 'August 01, 2018';

-- Count the number of apps in each category, e.g. “Family | 1972”.
SELECT category, COUNT(*) FROM analytics GROUP BY category;

SELECT MAX(reviews) FROM analytics;

-- Find the top 5 most-reviewed apps and the number of reviews for each.
SELECT app_name FROM analytics ORDER BY reviews DESC  LIMIT 5;

-- Find the app that has the most reviews with a rating greater than equal to 4.8.
SELECT app_name, price, rating FROM analytics WHERE rating >= 4.8;
...
SELECT id, app_name, reviews FROM analytics WHERE price BETWEEN 0.10 AND 1.00 ORDER BY reviews DESC LIMIT 10;
